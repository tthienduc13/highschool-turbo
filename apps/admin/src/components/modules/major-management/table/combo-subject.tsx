...
export interface ComboboxSubjectProps {
  setSubjects: React.Dispatch<React.SetStateAction<string[]>>;
}

export function ComboboxSubject({ setSubjects }: ComboboxSubjectProps) {
  ...
  const [subjectList, setSubjectList] = useState<MasterCourse[]>([]);
  ...
  const handleSelectedValues = (value: string) => {
    ...
    setSubjects(newSelectedValues);
  };

  const { data: tags, isLoading } = useMasterCoursesQuery({
    pageSize: pageSize,
    pageNumber: page,
  });
  ...
  useEffect(() => {
    if (tags?.data) {
      if (page === 1) {
        setSubjectList(tags.data);
      } else {
        setSubjectList((prev) => [...prev, ...tags.data]);
      }
                ...
                {subjectList.map((option) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <span>{option.masterSubjectName}</span>
                    </CommandItem>
                  );
                })}
              </CommandGroup>
      ...
      <div className="flex flex-wrap gap-2">
        {selectedValues.length > 0 &&
          selectedValues.map((subject) => (
            <Badge key={subject}>
              {subjectList.find((s) => s.id === subject)?.masterSubjectName ||
                "Unknown Subject"}
            </Badge>
          ))}
      </div>
