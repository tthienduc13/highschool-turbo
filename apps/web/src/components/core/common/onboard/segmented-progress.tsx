        ...
        <div className="flex flex-row gap-2 h-12 items-center">
            {Array.from({ length: steps }).map((_, i) => (
                <Segment
                    ...
                    onHover={(hover) => {
                        if (!clickable || (disableFrom && i >= disableFrom))
                            return;
    ...
    return (
        ...
        >
            <div
                style={{ height: height }}
                className={cn(
                    "bg-gray-100 dark:bg-gray-800/50 w-full rounded-full relative overflow-hidden transition-all duration-200 ease-cubic-ease",
                    clickable &&
            ...
            >
                <motion.div
                    initial={
                        step >= currentStep ? { width: 0 } : { width: "100%" }
                    }
                    animate={
                        step > currentStep ? { width: 0 } : { width: "100%" }
                    }
