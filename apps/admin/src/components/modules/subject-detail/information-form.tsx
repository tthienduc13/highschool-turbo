...
export const InformationForm = ({
  ...
  const form = useForm<z.infer<typeof formSchema>>({
    ...
    defaultValues: { information: initialData?.information ?? "" },
  });

  const { isSubmitting, isValid } = form.formState;

  const editInformation = useEditCourseMutation();

  const onSubmit = async (values: z.infer<typeof formSchema>) => {
    try {
      await editInformation.mutateAsync(
        {
          id: course.id,
      ...
      {!isEditing && (
        <p className="mt-2 whitespace-pre-wrap break-words text-sm dark:text-gray-300">
          {initialData?.information || (
            <span className="italic text-gray-500">No information</span>
          )}
        </p>
      )}
      {isEditing && (
