...
import { addOrUpdatePlayer, setPlayers, updatePlayerInfoOnList } from "../../../../store/slice/players-slice"
    ...
    const { channel } = useChannel(`room:${roomId}`, 'message-game', (message) => {
        ...
        switch (data.Type) {
            ...
            case RoomEventType.PlayerKicked: {
                dispatch(setPlayers(players.filter((player) => player?.id !== data.Data)));
                return
            }
            case RoomEventType.UpdatePlayerInfo: {
                dispatch(updatePlayerInfoOnList({
                    id: data.Data.Id,
                    avatar: data.Data.Avatar
                }));
                return;
    ...
    return (
        ...
        >
            ...
            <div className="relative z-10 flex flex-col items-center">
                ...
                <LobbyPlayer count={players.length} players={[...players]} roomId={roomId} />
                <LobbySetting />
            </div>
        </div>
    )
