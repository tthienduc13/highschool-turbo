...
export default function PlayerJoinModule() {
  ...
  const { mutateAsync: checkRoomMutate, isPending: checkRoomPending } =
    useCheckRoomMutation();
  const { mutateAsync: joinRoomMutate, isPending: joinRoomPending } =
    useJoinRoomMutation();
  ...
  const HandleJoinRoom = async () => {
    const res = await joinRoomMutate({
      data: {
        avatar:
  ...
  return (
    <>
      ...
      <motion.div
        ...
        hidden={isRoomExist}
        className="absolute left-[40%] top-[40%] transform text-center text-6xl font-extrabold text-white"
      >
        <h3 className="my-6">Coolket</h3>
        <div className="flex items-center space-x-2">
          <input
            type="text"
            className="shadow-inset-gray-shadow h-[10vh] w-[16vw] rounded-xl border-none bg-white px-4 pb-3 text-center text-2xl text-gray-600 outline-none"
            value={id}
          ...
          <ButtonKet
            ...
            className="h-[10vh] text-center text-black"
            heightShadow="-10px"
