...
import { EvaluatedFalse, EvaluatedTrue } from "../../study-set-test/evaluated";
import { MatchData } from "../types";

import { CardProps } from "./common";

import { InteractivePointerSensor } from "@/lib/sensor";
import { rememberedWord } from "@/utils/terms";
...
const Header: React.FC<Pick<CardProps, "i">> = ({ i }) => {
  const { question } = useCardRememberedSelector<MatchData>(i);
...
export const InteractiveMatchCard: React.FC<CardProps> = ({ i }) => {
  const { question, data, answer } = useCardRememberedSelector<MatchData>(i);
  ...
  const options = data.terms.filter(
    (t) => !answer.find((a) => a.term === t.contentId),
  );
      ...
      <div className="flex flex-col gap-2">
        ...
        <div className="mt-4 flex flex-col gap-2">
          <div className="flex flex-row flex-wrap gap-3">
            {options.map((term) => (
              <Draggable key={term.contentId} id={term.contentId}>
                <ExternalWrapper id={term.contentId}>
                  <div className="whitespace-pre-wrap">
                    <ScriptFormatter>
                      {rememberedWord(question.answerMode, term, "answer")}
                    </ScriptFormatter>
                  </div>
                </ExternalWrapper>
              </Draggable>
              ...
              <React.Fragment key={id}>
                ...
                >
                  <Droppable
                    ...
                    id={term.contentId}
                  >
                    {!!getInZone(term.contentId) && (
                      <Draggable id={getInZone(term.contentId)!.term}>
                        <InternalWrapper
                          id={getInZone(term.contentId)!.term}
                          onRemove={() => clearZone(term.contentId)}
                        >
                          <div className="whitespace-pre-wrap">
                            {" "}
                            <ScriptFormatter>
...
const ResultsMatchCard: React.FC<CardProps> = ({ i }) => {
  const { question, data, answer, remarks } =
    useCardRememberedSelector<MatchData>(i);
  ...
  const getAnswer = (id: string) => {
    ...
    return data.terms.find((t) => t.contentId === termId);
  };
      ...
      <div className="mt-4 flex flex-col gap-8">
        {data.zones.map((term, i) => (
          <React.Fragment key={i}>
            <div key={term.contentId} className="flex flex-col gap-6">
              <div className="flex w-full flex-row items-center justify-between gap-4">
              ...
              <div
                className={cn(
                  ...
                  evaluateZone(term.contentId) || !getAnswer(term.contentId)
                    ? "lg:grid-cols-1"
                    : "lg:grid-cols-2",
                )}
              >
                {!evaluateZone(term.contentId) && (
                  <div className="flex flex-col gap-2">
                    <GenericLabel evaluation={evaluateZone(term.contentId)}>
                      {remark(term.contentId)}
                    </GenericLabel>
                    {!!getAnswer(term.contentId) && (
                      <EvaluatedFalse>
                        {rememberedWord(
                          question.answerMode,
                          getAnswer(term.contentId)!,
