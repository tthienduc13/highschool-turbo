...
export const TestCardGapRaw = ({
    ...
    > = ({ children, rounded = "full", skeletonHeight = "18px" }) => {
        ...
        return (
            <div className="flex h-[21px] items-center">
                <Skeleton
                    className={cn(`rounded-${rounded} h-${skeletonHeight}`)}
                />
            </div>
        );
    };
    const HeadingWrapper: React.FC<React.PropsWithChildren> = ({
        children,
    }) => {
        if (!skeleton) return <>{children}</>;
    ...
    return (
        <div
            className={cn(
                "sm:px-[36px] flex w-full flex-row gap-4 px-5 md:px-[34px]",
        ...
        >
            <div className="relative min-h-12 w-[3px]">
                ...
                >
                    {(type === "question" || type === "finish") && (
                        <div className="sm:hidden sm:w-[3px] dark:bg-gray-750 absolute -top-4 left-0 -z-10 block h-20 w-0.5 bg-gray-200" />
                    )}
                </div>
                <div
                    className={cn(
            ...
            {title && (
                <div className="sm:pr-[26px] flex w-full flex-col-reverse items-start justify-start pr-[5px] md:flex-row md:justify-between md:pr-[34px]">
